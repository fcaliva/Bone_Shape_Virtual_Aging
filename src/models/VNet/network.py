# Credits: Initial code from github.com/MiguelMonteiro/VNet-Tensorflow
...
class VNet(object):
    ...
    def network_fn(self, x, keep_prob):
        ...
        spatial_rank = self.get_spatial_rank(x)
        with tf.variable_scope(f'{name}/input_layer'):
            if input_channels == 1:
        ...
        for l in range(self.num_levels):
            with tf.variable_scope(f'{name}/encoder/level_' + str(l + 1)):
        ...
        with tf.variable_scope(f'{name}/bottom_level'):
            x = self.convolution_block(x, self.bottom_convolutions, keep_prob)

        for l in reversed(range(self.num_levels)):
            with tf.variable_scope(f'{name}/decoder/level_' + str(l + 1)):
                f = features[l]
        ...
        with tf.variable_scope(f'{name}/output_layer'):
